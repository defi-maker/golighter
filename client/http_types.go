...
type BlockQueryType string

const (
	BlockQueryByCommitment BlockQueryType = "commitment"
	BlockQueryByHeight     BlockQueryType = "height"
)

// ServerStatus represents the response of the root status endpoint
...
// SimpleOrder captures the lightweight representation returned by order book order queries
...
// OrderBookOrdersResponse represents the response from /api/v1/orderBookOrders
...
// TxSummary mirrors the lightweight transaction payload embedded in block responses
...
// BlockInfo represents a single block returned by the blocks endpoint
...
// BlocksResponse models /api/v1/blocks and /api/v1/block responses
...
// CurrentHeightResponse captures the payload returned by /api/v1/currentHeight
...
// AnnouncementsResponse wraps a list of announcements
type AnnouncementsResponse struct {
	ResultCode
	Announcements []Announcement `json:"announcements"`
}

// FastBridgeInfoResponse models the fast bridge limit payload
type FastBridgeInfoResponse struct {
	ResultCode
	FastBridgeLimit string `json:"fast_bridge_limit"`
}

// L1MetadataResponse exposes metadata linked to an L1 address
...
// ReferralPointEntry represents a referral record in the referral points response
type ReferralPointEntry struct {
	L1Address             string  `json:"l1_address"`
	TotalPoints           float64 `json:"total_points"`
	WeekPoints            float64 `json:"week_points"`
	TotalRewardPoints     float64 `json:"total_reward_points"`
	WeekRewardPoints      float64 `json:"week_reward_points"`
	RewardPointMultiplier string  `json:"reward_point_multiplier"`
}

// ReferralPointsResponse aggregates referral point statistics for an account
type ReferralPointsResponse struct {
	ResultCode
	Referrals                        []ReferralPointEntry `json:"referrals"`
	UserTotalPoints                  float64              `json:"user_total_points"`
	UserLastWeekPoints               float64              `json:"user_last_week_points"`
	UserTotalReferralRewardPoints    float64              `json:"user_total_referral_reward_points"`
	UserLastWeekReferralRewardPoints float64              `json:"user_last_week_referral_reward_points"`
	RewardPointMultiplier            string               `json:"reward_point_multiplier"`
}

// EnrichedTransaction captures the detailed transaction payload from tx lookups
...
// EnrichedTxResponse models the payload returned by /api/v1/txFromL1TxHash
type EnrichedTxResponse struct {
	ResultCode
	EnrichedTransaction
}
